...
describe('GET /api/latency', () => {
  ...
  it('returns 400 for invalid URL strings', async () => {
    const req = new NextRequest('http://localhost/api/latency?url=not-a-real-url');
    const res = await GET(req);
    expect(res.status).toBe(400);
    const data = await res.json();
  ...
  it('returns 400 for invalid protocols', async () => {
    const req = new NextRequest('http://localhost/api/latency?url=file:///etc/passwd');
    const res = await GET(req);
    expect(res.status).toBe(400);
    const data = await res.json();
  ...
  it('returns 403 for SSRF block (not in allowlist)', async () => {
    const req = new NextRequest('http://localhost/api/latency?url=https://169.254.169.254/latest/meta-data/');
    const res = await GET(req);
  ...
  it('returns latency data for a valid allowlisted url', async () => {
    ...
    const req = new NextRequest('http://localhost/api/latency?url=https://www.vestcodes.co');
    const res = await GET(req);
